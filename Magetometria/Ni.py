...
print("Pozostałość magnetyczna Ni", Mh_up(0))

Hm_up = interp1d(range_y_up, range_x_up,  kind = 'linear')
print("Pole koercji Ni: ", Hm_up(0))


plt.figure(dpi=200)
plt.plot(HT_mu0, MH_Am, ".", color = "black", ms = 0.5, label = "Dane")
plt.plot(x_up,Mh_up(x_up), "--", lw = 0.5, label = "Interpolacja")
plt.plot(0,Mh_up(0) , "r+", ms = 3, label = "Remanencja B$_r$")
plt.plot(Hm_up(0)+0.0002, 0, "g+", ms = 3, label = "Pole koercji H$_c$")
plt.grid()
plt.legend()
plt.xlabel("Natężenie pola H${\mu_0}$ [T]")
plt.ylabel("Magnetyzacja próbki M [A/m]")
plt.savefig("plots/Ni_interp")

plt.figure(dpi=200)
plt.plot(HT_mu0, MH_Am, ".", color = "black", ms = 0.5, label = "Dane")
plt.plot(x_up,Mh_up(x_up), "--", lw = 0.5, label = "Interpolacja")
plt.plot(0,Mh_up(0) , "r+", ms = 6, label = "Remanencja B$_r$")
plt.plot(Hm_up(0)+0.0002, 0, "g+", ms = 6, label = "Pole koercji H$_c$")
plt.grid()
plt.xlabel("Natężenie pola H${\mu_0}$ [T]")
plt.ylabel("Magnetyzacja próbki M [A/m]")
plt.xlim([-0.01, 0.01])
...
plt.savefig("plots/Ni_max_and_min")


# Bc i Hr
sM_emu_Am = sM_emu*rho_Ni*1000/m_Ni #[A/m]

print("Pozostałość magnetyczna Niklu:", ((Mh_up(0)-Mh_low(0))/2),"$\pm$", np.mean(sM_emu_Am), "[A/m]")
print("Pole koercji Niklu: ", (-(Hm_up(0)-Hm_low(0))/2),"$\pm$", np.mean(sHT_mu0), "[T]")
...
Mh_mean_bohr = (Mh_mean*M_A_Ni*m_Ni)/(m_Ni*N_A*mu_b*rho_Ni*1000)
...
print("Namagnesowaniem nasycenia Ms Niklu: ", Ms_fited,"$\pm$",pcov[0][0])
...
plt.grid()
plt.xlabel("Natężęie pola H${\mu_0}$ [T]")
plt.ylabel("Różnica magnetyzacji M [A/m]")
plt.savefig("plots/Ni_E")